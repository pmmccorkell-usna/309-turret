...
mot_min_drive = 63000 #21000
mot_threshhold = 0.3

enc = rotaryio.IncrementalEncoder(board.GP15,board.GP14,4)

encoder_counts_per_rev = 3120
#3050 #3140 #3080 #3040 #2500 #1250 #5000
...
def position_control(target_position,gains,sample_time=3.0):
	...
	while(q):
		...
		if (len(encoder_data)>1):
			...
			print("%d, deg: %.2f, error: %.2f, pwm_theoretical: %.2f, pwm_actual: %.2f, dir: %d" %((now-start_time), pos, error, pwm_val, mot_pwm.duty_cycle, mot_dir.value))
			encoder_data.pop(0)
...
# Under REPL, import code has __name__ 'code'
if __name__ == 'code':
	import sys
	print('REPL detected')
...
#	position_control(-120,[4000,2000,0],5) #with min mot threshold at 9000
#	position_control(desired_angle,4000,3) #with min mot threshold at 10000
#	mot_pwm.duty_cycle = 58000
#	mot_dir.value = 0
#	enc_readback()
	print("enter data")
	print(">>")
	while(1):
		if supervisor.runtime.serial_bytes_available:
			...
			try:
				data=float(data)
				position_control(data,4000,3)
			except:
				print('DataType Error: input was not a number.')
			print(data)
			sleep(0.1)
